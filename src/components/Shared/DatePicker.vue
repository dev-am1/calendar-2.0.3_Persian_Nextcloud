...
export default {
	...
	data() {
		return {
			...
			momentFormat: {
				// Date to String
				stringify: (date) => {
					return date ? moment(date).locale('fa').format('LL') : ''
				},
				// String to Date
				parse: (value) => {
					return value ? moment(value, 'LL').locale('fa').toDate() : null
				},
			},
			showTimePanel: false,
		}
	...
	methods: {
		 ...
		 */
		stringify(date) {
			const formattedDate = momentj(date).locale('fa').format('L')
			const formattedTime = momentj(date).locale('fa').format('LT')
		...
		parse(value) {
			if (this.isAllDay) {
				...
				return momentj(matches[1], 'L', this.locale).toDate()
			} else {
				let format
				...
				return momentj(dateMatches[1] + ' ' + timeMatches[1], 'L LT', this.locale).toDate()
			}
		},
	},
}
