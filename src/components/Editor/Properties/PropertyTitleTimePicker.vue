	...
	computed: {
		...
		formattedStart() {
			if (this.isAllDay) {
				return this.$t('calendar', 'from {startDate}', {
					startDate: moment(this.startDate).locale('fa').format('L'),
					endDate: moment(this.endDate).locale('fa').format('L'),
				})
			}

			return this.$t('calendar', 'from {startDate} at {startTime}', {
				startDate: moment(this.startDate).locale('fa').format('L'),
				startTime: moment(this.startDate).locale('fa').format('LT'),
			})
		},
		/**
		 *
		...
		formattedEnd() {
			...
			return this.$t('calendar', 'to {endDate} at {endTime}', {
				endDate: moment(this.endDate).locale('fa').format('L'),
				endTime: moment(this.endDate).locale('fa').format('LT'),
			})
		},
		/**
		 * @returns {Boolean}
